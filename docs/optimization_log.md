...
### 1. 自我反思 (Self-Reflection)
*   **长上下文洞察瓶颈**：随着历史数据增加，简单的 `Counter` 统计可能无法捕捉“条件性规律”（如：周一的打车费通常关联项目 A，周五关联项目 B）。目前的模式挖掘是无监督且扁平的。
*   **异常回滚联动不足**：逻辑回撤（`logical_revert`）目前仅在查询层过滤，没有联动 `KnowledgeBase`。如果一笔被回撤的交易之前已经更新了供应商的“稳定度”，回撤时应当扣减相应的信任分。
*   **兼容性深度不足**：目前的导出仅支持分录，缺乏对“附件关联关系”的导出（如导出时附带原始单据的文件名映射，以便 ERP 勾稽）。

### 2. 优化方案 (Optimization Plan)
*   **[长上下文增强]**：在 `DBQueries` 中引入时间序列特征提取（Time-Series Feature Extraction），按星期、时段对历史记录进行聚类，提供更精准的行为模式。
*   **[信任分联动回滚]**：重构 `DBHelper` 或相关事务逻辑，当触发 `logical_revert` 时，自动修正 `KnowledgeBase` 中的 `consecutive_success` 与 `hit_count`。
*   **[导出附件索引]**：在导出包中增加 `attachment_manifest.json`，记录凭证与原始文件（OCR 结果）的映射。
