...
### 1. 自我反思 (Self-Reflection)
*   **长上下文稳定性**：在处理超长上下文（大数据量供应商历史）时，LLM 可能会因为 Token 限制或 Context Window 溢出而丢失早期关键特征。目前的 `pattern_insight` 仅统计了最近 12 个月的记录，对长期资产摊销等跨年特征感知不足。
*   **异常回滚死角**：当前回滚是基于全库快照的物理回撤，虽然保险但成本高（需停机覆盖）。缺乏一种“影子回撤”机制，即在不改变物理数据的情况下，通过逻辑标记使某些错误分录在报表层不可见。
*   **兼容性拼图缺失**：已完成 QB/SAP/金蝶适配，但对于“用友（yonyou）”这一中国市场份额巨大的系统仍未提供原生导入支持。

### 2. 优化方案 (Optimization Plan)
*   **[长上下文增强]**：在 `DBQueries` 中引入时间跨度探测（Time-Span Detection），自动根据记录密度调整挖掘窗口，确保长期负债或固定资产特征被优先保留。
*   **[数据保护升级]**：在 `transactions` 表中增加 `logical_revert` 状态，并重构所有 View 以过滤此类记录，实现秒级业务回撤。
