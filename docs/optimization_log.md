...
### 2. 优化方案 (Optimization Plan)
*   **[数据保护闭环]**：在 `DBMaintenance` 中实现 `rollback_to_snapshot` 方法，允许系统检测到致命错误（如全局不平衡或人为误操作）时，通过 API 触发物理回滚。
...
## 迭代 12 (Iteration 12) - [2025-03-24]

### 1. 自我反思 (Self-Reflection)
*   **长上下文“因果”特征挖掘的深度不足**：虽然有了简单的关联供应商识别，但系统仍无法量化这种关联的“置信度”和“因果方向”。
*   **一致性保护的物理开销**：禁止 DELETE 触发器虽然保护了证据，但随着时间推移，废弃数据（已回撤记录）会挤占大量 B-Tree 空间，导致查询性能缓慢下降。
*   **兼容性 Schema 验证的局限性**：目前的 Schema 校验仅支持简单的字段存在性检查，缺乏对字段内容（如：科目编码是否符合目标系统的会计科目表）的深度校验。

### 2. 优化方案 (Optimization Plan)
*   **[长上下文增强]**：在 `DBQueries` 中引入条件概率分析（Conditional Probability），识别供应商 A 出现后 B 出现的概率 P(B|A)，并作为洞察注入。
*   **[数据治理加固]**：在 `DBMaintenance` 中引入 `purge_reverted_to_archive` 方法，允许管理员将过期的逻辑回撤记录物理迁移至独立的 `archive.db`，保持主库轻量。
*   **[兼容性 Schema 进阶]**：在 `QB_SAP_Exporter` 中引入 External-Lookup 机制，支持在导出前校验科目编码是否在预设的“合法科目表”中。
