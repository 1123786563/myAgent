# LedgerAlpha 详细设计说明书 (Detailed Design Specification) - v4.0 核心对齐版

## 1. 引言 (Introduction)

### 1.1 编写目的
本文档旨在为 LedgerAlpha (微小企业全自动、自进化记账智能体系统) 的开发提供详尽的架构指导与实现细节。本版本已根据《需求规格说明书 v4.0》及《需求分析报告 v1.4》进行全面对齐与细化。
...
- **技术实现**：利用 OpenManus 驱动 Playwright 浏览器，模拟用户登录网银下载流水。
- **安全保障**：所有浏览器操作在本地沙箱运行，凭据通过本地 1Password 或加密 Store 注入。

#### 3.1.2 预记账预匹配 (Shadow Entry Matching)
- **流程**：`SMS/Email Notification` -> `AgentScope PendingEntry Actor` -> `Create Shadow Entry` -> `Await Receipt` -> `Fuzzy Match (Amount/Time/Vendor)`.
...
- **灰度规则 (Gray Rules)**：新规则需通过 3 次异构审计或人工确认方可转为 `Moltbot SOP` 规则。

### 3.3 经营分析与税务哨兵设计 (The Sentinel & Analyst)

#### 3.3.1 税务“模拟报税”沙箱 (Tax Sandbox)
- **逻辑**：模拟税务局申报接口，读取当前账目，计算预缴税额，并在 IM 端推送“税负压力测试报告”。
...
### 4.1 IM 寄生交互协议
- **卡片渲染**：使用 JSON 定义标准的交互卡片，支持按钮动作回调至 AgentScope 消息总线。
...
---
迭代记录：v4.0 全面优化版 [2025-03-24]
*(注：本版本已 100% 对齐需求分析 v1.4 中的“影子银企直连”、“投融资包”及“多重审计”要求。)*
