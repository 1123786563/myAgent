...
class MockOpenManusLLM(BaseLLM):
    ...
    def _get_file_mtime(self) -> float:
        try: return os.path.getmtime(self.kb_path)
        except OSError: return 0.0

    def _load_knowledge_base(self) -> dict:
        default_kb = {
            "aliyun": {"category": "技术服务费", "reason": "Cloud computing service provider"},
            "aws": {"category": "技术服务费", "reason": "Cloud infrastructure"},
            "starbucks": {"category": "业务招待费", "reason": "Coffee chain, likely client meeting"},
            "didi": {"category": "差旅费-交通费", "reason": "Ride hailing service"},
            "apple": {"category": "办公设备", "reason": "Electronics hardware"},
            "unknown": {"category": "杂项支出", "reason": "Low confidence classification"},
        }
        try:
            import yaml
            if os.path.exists(self.kb_path):
                with open(self.kb_path, "r", encoding="utf-8") as f:
                    ...
                    if data and "vendors" in data:
                        kb = {v.get("keyword", "").lower(): {"category": v.get("category", "杂项支出"), "reason": v.get("reason", "Loaded from KB")} for v in data["vendors"] if v.get("keyword")}
    ...
    def generate_response(self, prompt: str, system_role: str = "assistant") -> dict:
        ...
        matched_key = next((k for k in self.knowledge_base if k in prompt_lower), "unknown")
